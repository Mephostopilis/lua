...
static void
on_connected(struct lua_socket *so) {
	struct lua_gate *g = so->g;
	struct lua_State *L = g->L;
...
static void
on_disconnected(struct lua_socket *so) {
	struct lua_gate *g = so->g;
	struct lua_State *L = g->L;
	lua_pushlightuserdata(L, so);
	lua_rawget(L, LUA_GLOBALSINDEX);
	lua_getfield(L, -1, "disc");
...
static void
on_recv(struct lua_socket *so) {
	struct lua_gate *g = so->g;
	struct lua_State *L = g->L;
	lua_pushlightuserdata(L, so);
	lua_rawget(L, LUA_GLOBALSINDEX);
	lua_getfield(L, -1, "recv");  // func
	if (so->protocol == PROTOCOL_TCP) {
		if (so->header == HEADER_PG) {
			uint16_t sz = 0;
			unpackbH(so->bhead, 2, &sz);
			while (so->btail - so->bhead >= sz + 2) {
...
rebase(lua_State *L, struct lua_gate *g, struct lua_socket *so) {
	if (so->cap - (so->buffer - so->btail) <= 100) {
