...
{
template <>
struct hash<weak_ptr<Chestnut::EntitasPP::Entity>>
{
	std::size_t operator()(const weak_ptr<Chestnut::EntitasPP::Entity>& ptr) const
	{
		return hash<unsigned int>()(ptr.lock()->GetUuid());
	}
};

bool operator ==(weak_ptr<Chestnut::EntitasPP::Entity> left, weak_ptr<Chestnut::EntitasPP::Entity> right);
