...
static int linsert(lua_State *L) {
  ...
  int n = lua_gettop(L);
  if (n < 3) { return luaL_error(L, "The param more than 3"); }
  if (lua_isnil(L, 3)) { return luaL_error(L, "The param #3 is nil"); }
  lua_rawgeti(L, 1, 0);
  lua_Integer sparselen = luaL_checkinteger(L, -1);
  if (idx == sparselen + 1) {
    lua_pushvalue(L, 3);
...
static int lerase(lua_State *L) {
  luaL_checktype(L, 1, LUA_TTABLE);
  if (lua_isnil(L, 2)) { return luaL_error(L, "The param #3 is nil"); }
...
** @return 0
*/
static int leraseat(lua_State *L) {
  luaL_checktype(L, 1, LUA_TTABLE);
  lua_Integer idx = luaL_checkinteger(L, 2);
  if (idx <= 0) {
    return luaL_error(L, "The index should be positive (%d)", (int)idx);
...
** @return 1 (index)
*/
static int lpush_back(lua_State *L) {
  luaL_checktype(L, 1, LUA_TTABLE);
...
** @return 0
*/
static int lpop_back(lua_State *L) {
  luaL_checktype(L, 1, LUA_TTABLE);
  lua_rawgeti(L, 1, 0);
  lua_Integer sparselen = luaL_checkinteger(L, -1);
  if (sparselen > 0) {
...
** @return 1 (index)
*/
static int lindexof(lua_State *L) { return 0; }

static int quick_sort(lua_State *L, lua_Integer l, lua_Integer r) {
  if (l >= r) { return 0; }
...
static int lnewindex(lua_State *L) {
  ...
  lua_Integer sparselen = luaL_checkinteger(L, -1);
  if (idx > sparselen) {
          return luaL_error(L, "The index should be less then (%d)",
  (int)sparselen);
  }
  lua_pop(L, 1);
...
static int lnewvector(lua_State *L) {
  int n = lua_gettop(L);
...
LUAMOD_API int luaopen_chestnut_vector(lua_State *L) {
  luaL_checkversion(L);
