...
function cls:Push(nextContext)
    ...
    assert(not nextContext.visible)
    if not nextContext.visible then
        self._contextStack:push(nextContext)
        local traceback = debug.traceback
        local ok, err = xpcall(nextContext.OnEnter, traceback, nextContext)
        if not ok then
            log.error(err)
        end
...
function cls:PopAll()
    log.info('self._contextStack length ==', #self._contextStack)
