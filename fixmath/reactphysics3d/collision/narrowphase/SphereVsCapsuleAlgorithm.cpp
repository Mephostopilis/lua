    ...
    if (sphereSegmentDistanceSquare < sumRadius * sumRadius) {
        ...
        if (reportContacts) {
			...
			if (sphereSegmentDistanceSquare > MACHINE_EPSILON) {
				...
				decimal sphereSegmentDistance = rpSqrt(sphereSegmentDistanceSquare);
				...
				decimal cosA1 = rpAbs(capsuleSegment.x);		// abs(vec1.dot(seg2))
				decimal cosA2 = rpAbs(capsuleSegment.y);	    // abs(vec2.dot(seg2))
