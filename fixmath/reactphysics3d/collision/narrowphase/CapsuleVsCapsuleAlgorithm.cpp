		...
		if (t1 > decimal(0.0) && t2 > decimal(0.0)) {
            ...
            if (reportContacts) {
				...
				if (segmentsPerpendicularDistance > MACHINE_EPSILON) {
					...
					decimal cosA1 = rpAbs(seg2Normalized.x);		// abs(vec1.dot(seg2))
					decimal cosA2 = rpAbs(seg2Normalized.y);	    // abs(vec2.dot(seg2))
    ...
    if (closestPointsDistanceSquare < sumRadius * sumRadius) {
		...
		if (reportContacts) {
			...
			if (closestPointsDistanceSquare > MACHINE_EPSILON) {
				...
				decimal closestPointsDistance = rpSqrt(closestPointsDistanceSquare);
