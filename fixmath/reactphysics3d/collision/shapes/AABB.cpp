...
bool AABB::testRayIntersect(const Ray& ray) const {
    ...
    decimal adx = rpAbs(d.x);
    if (rpAbs(m.x) > e.x + adx) return false;
    decimal ady = rpAbs(d.y);
    if (rpAbs(m.y) > e.y + ady) return false;
    decimal adz = rpAbs(d.z);
    if (rpAbs(m.z) > e.z + adz) return false;
    ...
    // separating axis
    if (rpAbs(m.y * d.z - m.z * d.y) > e.y * adz + e.z * ady) return false;
    if (rpAbs(m.z * d.x - m.x * d.z) > e.x * adz + e.z * adx) return false;
    if (rpAbs(m.x * d.y - m.y * d.x) > e.x * ady + e.y * adx) return false;
