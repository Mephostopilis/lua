...
Vector3 Vector3::getOneUnitOrthogonalVector() const {
    ...
    Vector3 vectorAbs(rpAbs(x), rpAbs(y), rpAbs(z));
    int minElement = vectorAbs.getMinAxis();

    if (minElement == 0) {
        return Vector3(0.0, -z, y) / rpAbs(y*y + z*z);
    }
    else if (minElement == 1) {
        return Vector3(-z, 0.0, x) / rpSqrt(x*x + z*z);
    }
    else {
        return Vector3(-y, x, 0.0) / rpSqrt(x*x + y*y);
    }
