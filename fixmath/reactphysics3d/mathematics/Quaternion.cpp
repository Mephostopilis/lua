    ...
    if (trace < decimal(0.0)) {
        if (matrix[1][1] > matrix[0][0]) {
            if(matrix[2][2] > matrix[1][1]) {
                r = rpSqrt(matrix[2][2] - matrix[0][0] - matrix[1][1] + decimal(1.0));
                s = decimal(0.5) / r;
                ...
                r = rpSqrt(matrix[1][1] - matrix[2][2] - matrix[0][0] + decimal(1.0));
                s = decimal(0.5) / r;
            ...
            r = rpSqrt(matrix[0][0] - matrix[1][1] - matrix[2][2] + decimal(1.0));
            s = decimal(0.5) / r;
...
Quaternion Quaternion::slerp(const Quaternion& quaternion1,
    ...
    decimal theta = rpAcos(cosineTheta);

    // Compute sin(theta)
    decimal sineTheta = rpSin(theta);

    // Compute the two coefficients that are in the spherical linear interpolation formula
    decimal coeff1 = rpSin((decimal(1.0)-t)*theta) / sineTheta;
    decimal coeff2 = rpSin(t*theta) / sineTheta * invert;
...
void Quaternion::initWithEulerAngles(decimal angleX, decimal angleY, decimal angleZ) {
    ...
    const decimal sinX = rpSin(angle);
    const decimal cosX = rpCos(angle);

    angle = angleY * decimal(0.5);
    const decimal sinY = rpSin(angle);
    const decimal cosY = rpCos(angle);

    angle = angleZ * decimal(0.5);
    const decimal sinZ = rpSin(angle);
    const decimal cosZ = rpCos(angle);
