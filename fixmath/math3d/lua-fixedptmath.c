...
#include "fixedptmath3d.h"
	...
	static int
		...
		if (lua_isuserdata(L, 1)) {
			...
			tmp.x = fix16_from_int(luaL_optinteger(L, 1, 0));
			tmp.y = fix16_from_int(luaL_optinteger(L, 2, 0));
			tmp.z = fix16_from_int(luaL_optinteger(L, 3, 0));
		}
	...
	static int
		...
		vec3->x = fix16_from_int(luaL_optinteger(L, 2, 0));
		vec3->y = fix16_from_int(luaL_optinteger(L, 3, 0));
		vec3->z = fix16_from_int(luaL_optinteger(L, 4, 0));
		lua_settop(L, 1);
		return 1;
	}
	...
	static int
		...
		struct vector3 *vec3 = (struct vector3 *)check_userdata(L, 1);
		lua_pushnumber(L, fix16_to_float(vec3->x));
		lua_pushnumber(L, fix16_to_float(vec3->y));
		lua_pushnumber(L, fix16_to_float(vec3->z));
		return 3;
	}
	...
	static int
		...
		fix16_t v = vector3_dot(a, b);
		float f = fix16_to_float(v);
		lua_pushnumber(L, f);
		return 1;
	}
	...
	static int
		...
		vector3_lerp(v, a, b, fix16_from_float(f));
		lua_settop(L, 1);
		return 1;
	}
...
#define lvec4_unpack lquat_unpack
	...
	LUAMOD_API int
		luaopen_fixmath_math3d(lua_State *L) {
		luaL_checkversion(L);
		lua_newtable(L);
		vector3(L);
