...
class Fix16 {
	public:
		...
		Fix16(const long long inValue) { value = fix16_from_int(inValue); }
		...
		const Fix16 operator-() const { Fix16 ret = *this; ret = 0 - ret; return ret; }
		...
		const Fix16 operator() (const Fix16 &other)  const { return other; }
		const Fix16 operator() (const fix16_t other) const { return Fix16(other); }
		const Fix16 operator() (const double other)  const { return Fix16(other); }
		const Fix16 operator() (const float other)   const { return Fix16(other); }
		const Fix16 operator() (const long long other)   const { return Fix16(other); }
		const Fix16 operator() (const int16_t other) const { return Fix16(other); }
