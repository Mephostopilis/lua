...
lnew(lua_State *L) {
	...
	init_lib(L);
	struct lua_gate *g = (struct lua_gate *)lua_newuserdata(L, sizeof(*g) + MAX_SOCKET_NUM * sizeof(struct lua_socket));
	memset(g, 0, sizeof(*g) + MAX_SOCKET_NUM * sizeof(struct lua_socket));
	g->head = NULL;
