...
function cls:SendStep()
    ...
    NetworkMgr:getInstance().client:send_request('step', request)
end

function cls:SendRestart()
    local request = {}
    request.idx = self._context.rule.myidx
    NetworkMgr.client:send_request("restart", request)
end
-- endregion
...
function cls:OnTakeReady(requestObj)
    -- 转移状态
    ...
    local request = {}
    request.idx = myEntity.player.idx
    NetworkMgr:getInstance().client:send_request('ready', request)
...
function cls:OnShuffle(requestObj)
    if self._context.rule.gamestate ~= GameState.SHUFFLE then
        local obj  = requestObj
        self._context.rule.oknum = 0
...
function cls:OnTakeXuanQue(requestObj)
    ...
    -- local gameUIModule = _appContext.U.GetModule<GameUIModule>()
...
function cls:OnMCall(requestObj)
    local obj = requestObj
    self._context.rule.gamestate = GameState.MCALL
    self._gameSystems.deskSystem.ShowCountdown(obj.countdown)
    ...
    local entity = self._gameSystems.netIdxSystem:FindEntity(obj.opcodes.idx)
    entity.playerCard.call = call
    self._gameSystems.playerSystem.SetupCall(entity)
...
function cls:OnPeng(requestObj)
    ...
    local entity = _gameSystems.netIdxSystem:FindEntity(obj.idx)
    local dianEntity = _gameSystems.netIdxSystem:FindEntity(obj.dian)
    local cardEntity = _gameSystems.CardValueIndexSystem:FindEntity(obj.card)
    self._gameSystems.playerSystem.Peng(entity, dianEntity, cardEntity, obj.hor)
